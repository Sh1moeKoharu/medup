  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      return sdk.admin.draftOrder.retrieve(draftOrderId, {
        ...
        console.warn('Failed to fetch draft order, clearing local ID:', error.message);
...
export const useResetDraftOrderLocalSession = () => {
  const queryClient = useQueryClient();

  return useMutation({
    mutationKey: ['draft-order', 'reset-local'],
    mutationFn: async () => {
      await Storage.deleteItemAsync(DRAFT_ORDER_ID_STORAGE_KEY);
    },
    onSettled: async () => {
      if (queryClient.isMutating({ mutationKey: ['draft-order'], exact: false }) === 1 || queryClient.isMutating({ mutationKey: ['draft-order'], exact: false }) === 0) {
        await queryClient.invalidateQueries({
          queryKey: ['draft-order'],
          exact: false,
        });
