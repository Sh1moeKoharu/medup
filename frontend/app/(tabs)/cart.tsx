        ...
        <View className="flex-1 flex-col gap-2">
          ...
          <View className="flex-row flex-wrap items-center gap-x-2 gap-y-1">
            ...
            <View className="flex-row gap-1">
              ...
              <Text className="text-sm">{isAutomatic ? 'Automático' : 'Código'}</Text>
            </View>
            {item.campaign && (
              <View className="flex-row gap-1">
...
export default function CartScreen({ isSidebar }: { isSidebar?: boolean }) {
  const settings = useSettings();
    ...
    return (
      <Layout className={`pb-6 ${isSidebar ? 'px-4 md:px-4 lg:px-4 xl:px-4' : ''}`}>
        ...
        <View className="flex-1 items-center justify-center gap-1">
          ...
          <Text className="text-gray-300">Añade productos para comenzar</Text>
        </View>
        <View className="flex-row gap-2">
          <Button
            variant="outline"
