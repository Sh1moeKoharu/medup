  ...
  return (
    <View
      ...
      onLayout={(event) => {
        ...
        if (event.target && typeof (event.target as any).measure === 'function') {
          (event.target as any).measure((x: number, y: number, w: number) => {
            handleWidth(w);
          });
        } else if (event.nativeEvent?.layout?.width) {
          handleWidth(event.nativeEvent.layout.width);
        }
      }}
    >
      <Carousel
  ...
  return (
    <ScrollView showsVerticalScrollIndicator={false} contentContainerClassName="pb-safe-offset-6">
      ...
      <View className="md:flex-row md:gap-6">
        ...
        <View className="-ml-4 mb-6 w-full max-w-xl overflow-hidden rounded-xl bg-gray-100 md:flex-1">
                    ...
                    items: [
                      {
                        ...
                        unit_price: (() => {
                          const matchingPrice = selectedVariant.prices?.find((p: any) => p.currency_code === currencyCode);
                          return matchingPrice?.amount !== undefined ? matchingPrice.amount : 0;
                        })()
                      },
                    ],
                  },
