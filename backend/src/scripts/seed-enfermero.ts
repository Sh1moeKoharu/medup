...
export default async function createEnfermero({ container }: { container: MedusaContainer }) {
  const userModuleService = container.resolve(Modules.USER);
  const authModuleService = container.resolve(Modules.AUTH);
  
  const email = "enfermero@pos.com";
  const password = "Password123#!";
  
  console.log(`Creando usuario enfermero con email: ${email}`);

  const existing = await userModuleService.listUsers({ email });
  if (existing.length > 0) {
    console.log("El usuario enfermero ya existe.");
    return;
  }
  ...
  console.log(`Usuario enfermero creado exitosamente.`);
  console.log(`-----------------------------------`);
